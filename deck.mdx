...
<Observe>
  { ({ checked, clean }) => (
    <Box style={{ marginBottom: "30px" }}> {
      checked.map(v => (<div style={{ display: 'inline-block', textAlign: 'center', marginRight: '10px', height: '80px', width: '50px' }}>{ (v == 1 && !clean) ? '⬇️' : ' ' }</div>))}
    </Box>
  ) }
</Observe>
...
* Blacklists of telephone numbers (used for blocking calls)
* These lists are **much** smaller than all possible telephone numbers
* Lists are too large to stay in memory, and need to be persisted
* Checking a persistent data store for every call is expensive
* Espeically true when we know most numbers will not be in a blacklist
...
# Bloom Filters Must Stay in Memory to be Used Correctly
...
* Hash count also needs to be stored - could just but an _int_ at the beggining of the filter
